    ...
    return (
        <div className="relative text-center bg-white bg-opacity-75 p-8 rounded-lg shadow-lg w-full max-w-xl">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col items-center">
                ...
                {message && <p className="mt-4 text-2xl text-gray-800 h-8">{message}</p>}

                {message.startsWith('Correct') ? (
                    <button type="button" onClick={handleNextWord} className="mt-4 px-6 py-3 bg-blue-500 text-white font-bold rounded-lg text-xl hover:bg-blue-700">
                        Next Word
                    </button>
                ) : (
                    <Controls
